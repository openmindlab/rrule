...
;(function (root, factory) {
  ...
  var dateutil = {
    ...
    convertToUTC: function(date, same) {
      var d = new Date(date);
      var nd = same ? date : new Date();
      ...
      nd.setUTCMilliseconds( d.getMilliseconds() );
      return nd;
    },
     ...
     */
    combine: function (date, time, useUTC) {
      time = time || date
      if ( useUTC ) {
   ...
   * @constructor
   */
  var RRule = function (options, noCache, extraOptions) {
    options = options || {}
    ...
    /* Fat: added extraOptions */
    this.extraOptions = {};
    var extraOptionsKeys = Object.keys(RRule.EXTRA_OPTIONS);
    extraOptionsKeys.forEach(function (key){
      this.extraOptions[ key ] = key in extraOptions ? extraOptions[key] : RRule.EXTRA_OPTIONS[ key ];
    }, this);
  ...
  RRule.prototype = {
    ...
    all: function (iterator) {
      if (iterator) {
        return this._iter(new CallbackIterResult('all', {}, iterator, this.extraOptions.useUTC))
      } else {
        var result = this._cacheGet('all')
        if (result === false) {
          result = this._iter(new IterResult('all', {}, this.extraOptions.useUTC))
    ...
    between: function (after, before, inc, iterator) {
      ...
      if (iterator) {
        return this._iter(new CallbackIterResult('between', args, iterator, this.extraOptions.useUTC))
      }
      var result = this._cacheGet('between', args)
      if (result === false) {
        result = this._iter(new IterResult('between', args, this.extraOptions.useUTC))
    ...
    before: function (dt, inc) {
      ...
      if (result === false) {
        result = this._iter(new IterResult('before', args, this.extraOptions.useUTC))
    ...
    after: function (dt, inc) {
      ...
      if (result === false) {
        result = this._iter(new IterResult('after', args, this.extraOptions.useUTC))
    ...
    */
    ...
    _cacheGet: function (what, args) {
      ...
      if (!cached && this._cache.all) {
        ...
        var iterResult = new IterResult(what, args, this.extraOptions.useUTC)
    ...
    _iter: function (iterResult) {
      ...
      */
      ...
      var year = this.extraOptions.useUTC ? dtstart.getUTCFullYear() : dtstart.getFullYear();
      var month = this.extraOptions.useUTC ? dtstart.getUTCMonth() + 1 : dtstart.getMonth() + 1;
      var day = this.extraOptions.useUTC ? dtstart.getUTCDate() : dtstart.getDate();
      var hour = this.extraOptions.useUTC ? dtstart.getUTCHours() : dtstart.getHours();
      var minute = this.extraOptions.useUTC ? dtstart.getUTCMinutes() : dtstart.getMinutes();
      var second = this.extraOptions.useUTC ? dtstart.getUTCSeconds() : dtstart.getSeconds();
      var weekday = this.extraOptions.useUTC ? dateutil.PY_WEEKDAYS[dtstart.getUTCDay()] : dateutil.getWeekday(dtstart);
        ...
        if (plb(bysetpos) && plb(timeset)) {
          ...
          for (i, j = 0; j < bysetpos.length; j++) {
            ...
            try {
              ...
              var res = dateutil.combine(date, time, this.extraOptions.useUTC )
          ...
          for (j = start; j < end; j++) {
            ...
            if (i !== null) {
              ...
              /* Fat: check if day is the same calculated day (change from Legal Hour) */
              if ( this.extraOptions.resetByDay && day > date.getDate() ) {
                if ( this.extraOptions.useUTC ) date.setUTCDate( day );
                else date.setDate( day );
              }
              for (k = 0; k < timeset.length; k++) {
                time = timeset[k]
                res = dateutil.combine(date, time, this.extraOptions.useUTC)
                if (until && res > until) {
                  this._len = total
                  return iterResult.getValue()
                } else if (res >= dtstart) {
        ...
        if (freq === RRule.YEARLY) {
          ...
          while (true) {
            ...
            if (div) {
              ...
              day += div
              fixday = true
              /* Fat: recalculate interval starting next day */
              if ( this.extraOptions.resetByDay ) {
                hour = this.extraOptions.useUTC ? dtstart.getUTCHours() : dtstart.getHours();
                minute = this.extraOptions.useUTC ? dtstart.getUTCMinutes() : dtstart.getMinutes();
                second = this.extraOptions.useUTC ? dtstart.getUTCSeconds() : dtstart.getSeconds();
              }
            }
   ...
   */
  var IterResult = function (method, args, useUTC) {
    this.init(method, args, useUTC)
  }

  IterResult.prototype = {
    constructor: IterResult,
    init: function (method, args, useUTC) {
      this.method = method
      this.args = args
      this.minDate = null
      this.maxDate = null
     ...
     */
    add: function (date) {
      var d = this._useUTC ? dateutil.convertToUTC(date, false) : date;
      this._result.push(d)
      return true
    },
    ...
    clone: function () {
      return new IterResult(this.method, this.args, this._useUTC)
    }
  }
   ...
   */
  var CallbackIterResult = function (method, args, iterator, useUTC) {
    ...
    this.add = function (date) {
      ...
      if (iterator(d, this._result.length)) {
        this._result.push(d)
        return true
      }
      return false
